...
function registerGetEnvironment(server) {
    server.tool('get_environment', 'Get variables for a specific environment in a Bruno collection (secrets are masked)', {
        ...
        if (!env) {
            ...
            return { content: [{ type: 'text', text: `Environment "${envName}" not found. Available: ${available}` }], isError: true };
        }
        ...
        return { content: [{ type: 'text', text: `Environment: ${env.name}\nVariables:\n${text}` }] };
    });
}
