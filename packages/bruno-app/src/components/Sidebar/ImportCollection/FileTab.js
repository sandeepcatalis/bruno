...
const FileTab = ({
  ...
  const processZipFile = async (zipFile) => {
    ...
    try {
      ...
      toastError(new Error('The ZIP file is not a valid Daffy collection'));
    } catch (err) {
      ...
      >
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
            Supports Daffy, OpenCollection, Postman, Insomnia, OpenAPI 3.x / Swagger 2.0, WSDL, and ZIP formats
          </p>
        </div>
      </div>
    </div>
